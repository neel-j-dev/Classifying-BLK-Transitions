...
def plot_probability_scatter(ax, temps: np.ndarray, probs: np.ndarray, labels: np.ndarray, t_c: float, true_tc: float | None):
    scatter = ax.scatter(
        temps,
        probs,
        c=probs,
...
def main():
    ...
    parser.add_argument("--dataset", type=Path, default=Path("../blt_dataset/train.npz"))
    ...
    plot_probability_scatter(ax_scatter, temps, probs, labels, t_c, true_tc)
